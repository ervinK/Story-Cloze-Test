    ...
    def compare(self, filename):
        ...
        with open('output_toPlot.csv', 'w', newline='') as f:
            ...
            for x in range(0,50):
                ...
                sims1 = self.model.docvecs.most_similar([s1_vector], topn=500)
                sim1Collect = self.getCosSums(sims1)
            
                self.s2[x] = filterSentences(self.s2[x])
                s2_vector = model.infer_vector(self.s2[x])
                sims2 = self.model.docvecs.most_similar([s2_vector], topn=500)
                sim2Collect = self.getCosSums(sims2)
                

                if self.decide[x] == 1:
                    writer.writerow({'id' : x, 'noending_sentence' : self.s2[x], 'ending_sentence' : self.s1[x], 'noending_similarity' : sim2Collect, 'ending_similarity' : sim1Collect})
                else:
                    writer.writerow({'id' : x, 'noending_sentence' : self.s1[x], 'ending_sentence' : self.s2[x], 'noending_similarity' : sim1Collect, 'ending_similarity' : sim2Collect})
