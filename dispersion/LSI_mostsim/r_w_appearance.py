...
"""
A feladat az, hogy meghatarozzuk, mely mondatok allnak a train setből legközelebb az összes val set mondathoz
"""
...
class LsiSimsCalc:
    ...
    def determineSims(self, topDepth):
        with open('appearances.csv', 'w', newline='') as f:
            ...
            for i in range(0,400):
                ...
                if self.dir[i] == 1:
                    ...
                    while iter < topDepth: # a top mennyit nezzuk hasonlosagi mercenek
                        try:
                            self.simCollectorNotEnding[self.processString(simlist2[iter])] += 1
                        except:
                            self.simCollectorNotEnding[self.processString(simlist2[iter])] = 0
                            self.simCollectorNotEnding[self.processString(simlist2[iter])] += 1
                        try:
                            self.simCollectorIsEnding[self.processString(simlist1[iter])] += 1
                        except:
                            self.simCollectorIsEnding[self.processString(simlist1[iter])] = 0
                            self.simCollectorIsEnding[self.processString(simlist1[iter])] += 1
                        iter += 1
                else:
                    ...
                    while iter < topDepth:
                        try:
                            self.simCollectorNotEnding[self.processString(simlist1[iter])] += 1
                        except:
                            self.simCollectorNotEnding[self.processString(simlist1[iter])] = 0
                            self.simCollectorNotEnding[self.processString(simlist1[iter])] += 1
                        try:
                            self.simCollectorIsEnding[self.processString(simlist2[iter])] += 1
                        except:
                            self.simCollectorIsEnding[self.processString(simlist2[iter])] = 0
                            self.simCollectorIsEnding[self.processString(simlist2[iter])] += 1
                        iter += 1
                print("The " + str(i) + ". round terminated")
